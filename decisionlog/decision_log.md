...
| 12/03 | Visibility model must behave correctly (worse visibility to higher risk) and stay bounded | A) Unnormalized score B) Multiplicative model C) **Normalized weighted sum \(V\in[0,1]\), scaling \(S=1-V\)** | Bounded outputs; monotonic behavior; tuning simplicity; explainability | Picked **C** because it guarantees \(V\) stays within **[0,1]**, and \(S\) increases as visibility drops. | AI drafted formulas; humans corrected **sign direction** and ensured bounds or clamping assumptions are clear. | Chor Yi |
